...
export default function App() {
  ...
  const labels = useMemo<LabelData[]>(() => (tab === 'single' ? [single] : expandBatch(batch)), [tab, single, batch]);
  const zpl = useMemo(() => zplForLabels(labels, config), [labels, config]);

  const setConfig = (c: Partial<LabelConfig>) => setState((s) => ({ ...s, config: { ...s.config, ...c } }));
      ...
      <main className="modern-app">
        <header className="app-topbar">
          <div className="app-title">
            ...
            <div><h1>Etykieciarka</h1><p>Zebra ZD421 · edytor WYSIWYG</p></div>
          </div>
          <div className="mode-switch">
            <button className={tab === 'single' ? 'active' : ''} onClick={() => { setTab('single'); setBatchOpen(false); }}>Jedna etykieta</button>
            <button className={tab === 'batch' ? 'active' : ''} onClick={() => { setTab('batch'); setBatchOpen(true); }}>Seria {tab === 'batch' ? `(${labels.length})` : ''}</button>
          </div>
        </header>
        ...
        {tab === 'batch' && batchOpen && (
          <div className="batch-drawer">
            <div className="drawer-heading">
              <div><strong>Generator serii</strong><span>Ustaw zakres, a edytor pokaże pierwszą etykietę z serii.</span></div>
        ...
        <LabelEditor data={previewLabel} config={config} onDataChange={tab === 'single' ? setSingle : () => undefined} onConfigChange={setConfig} />

        <section className="output-dock">
          <div className="output-heading">
            <div><strong>Druk i eksport</strong><span>{labels.length} {labels.length === 1 ? 'etykieta' : 'etykiet'} · {config.dpi} dpi</span></div>
            {tab === 'batch' && !batchOpen && <button className="secondary-action" onClick={() => setBatchOpen(true)}>Edytuj serię</button>}
          </div>
