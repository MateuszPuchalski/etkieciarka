...
/**
 * Umieszcza obiekt wyłącznie wewnątrz jego sekcji i przyciąga jego
 * lewą/górną krawędź, środek lub prawą/dolną krawędź do odpowiednich
 * punktów tej sekcji. Dzięki temu element nie snapuje do obcych obiektów.
 */
function placeInSection<T extends Box>(box: T, dx: number, dy: number, section: Box): T {
  ...
  const left = snapValue(x, [section.x]);
  const centerX = snapValue(x + box.w / 2, [sectionCx]);
  const right = snapValue(x + box.w, [section.x + section.w]);
  const dxLeft = Math.abs(left - x);
  const dxCenter = Math.abs(centerX - (x + box.w / 2));
  const dxRight = Math.abs(right - (x + box.w));
  const bestX = Math.min(dxLeft, dxCenter, dxRight);
  if (bestX <= SNAP_MM) {
    if (bestX === dxLeft) x = left;
    else if (bestX === dxCenter) x += centerX - (x + box.w / 2);
    else x += right - (x + box.w);
  }

  const top = snapValue(y, [section.y]);
  const centerY = snapValue(y + box.h / 2, [sectionCy]);
  const bottom = snapValue(y + box.h, [section.y + section.h]);
  const dyTop = Math.abs(top - y);
  const dyCenter = Math.abs(centerY - (y + box.h / 2));
  const dyBottom = Math.abs(bottom - (y + box.h));
  const bestY = Math.min(dyTop, dyCenter, dyBottom);
  if (bestY <= SNAP_MM) {
    if (bestY === dyTop) y = top;
    else if (bestY === dyCenter) y += centerY - (y + box.h / 2);
    else y += bottom - (y + box.h);
  }
...
export function computeLayout(cfg: LabelConfig, data: LabelData): LabelLayout {
  ...
  // Prawa sekcja ma stały wewnętrzny podział KOLUMNA / PÓŁKA.
